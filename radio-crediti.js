(()=>{
  ...
  document.documentElement.style.overflowX='hidden';document.body.style.overflowX='hidden';document.documentElement.style.maxWidth='100%';document.body.style.maxWidth='100%';document.documentElement.style.touchAction='pan-y pinch-zoom';document.body.style.touchAction='pan-y pinch-zoom';document.body.style.paddingBottom='calc(72px + env(safe-area-inset-bottom))';

  const style=document.createElement('style');
  style.textContent=`
    #crediti-radio-root{position:fixed;left:10px;bottom:calc(8px + env(safe-area-inset-bottom));max-width:calc(100vw - 20px);z-index:90;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#111}
  ...
  async function loadOffline(autoplay,advance=false){const available=await cachedTracks();if(!available.length){setStatus('Offline ainda não baixado');syncPlay();return false}if(advance)offlineIndex=(offlineIndex+1)%available.length;else offlineIndex=Math.min(offlineIndex,available.length-1);const t=available[offlineIndex];sourceMode='offline';switching=true;audio.src=t.url;audio.load();setStatus(`Offline • ${t.title}`);setTimeout(()=>switching=false,250);if('mediaSession'in navigator){try{navigator.mediaSession.metadata=new MediaMetadata({title:t.title,artist:'Rádio Crediti Offline',album:'Playlist grátis CC0'})}catch(_){}}if(autoplay)audio.play().catch(()=>setStatus('Toque em Play para ouvir offline'));return true}
  ...
  btnPlay.addEventListener('click',async()=>{setOpen(true);if(!audio.paused){audio.pause();return}if(!navigator.onLine){await loadOffline(true,false);return}if(sourceMode!=='live')sourceMode='live';if(!prepared)await fetchStations();if(prepared){if(!audio.src||sourceMode!=='live')loadLive(false);try{await audio.play()}catch(_){setStatus('Toque novamente em Play para ouvir')}}else{const ok=await loadOffline(true,false);if(!ok)setStatus('Rádio indisponível no momento')}});
  ...
  audio.addEventListener('play',()=>{syncPlay();if(sourceMode==='offline'){const t=OFFLINE_PLAYLIST.find(x=>x.url===audio.src);setStatus(`Offline • ${t?.title||'playlist variada'}`)}else setStatus('Tocando • programação variada')});
  ...
  audio.addEventListener('stalled',()=>{if(!navigator.onLine)switchToOffline();else setStatus('Reconectando…')});
  ...
  audio.addEventListener('playing',()=>{if(sourceMode==='live')setStatus('Tocando • programação variada')});
